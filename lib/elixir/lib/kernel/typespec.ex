defmodule Kernel.Typespec do
  ...
  defp typespec({:record, meta, [tag, field_specs]}, vars, caller, state) when is_atom(tag) and is_list(field_specs) do
    # We cannot set a function name to avoid tracking
  ...
  defp typespec({:record, _meta, [_tag, field_specs]}, _vars, caller, _state) when is_list(field_specs) do
